...
keys_binds = [
    # move focus window
    Key([super, "control"], "j", lazy.layout.shuffle_left(), desc="Move window to the left"),
    Key([super, "control"], "l", lazy.layout.shuffle_right(), desc="Move window to the right"),
    Key([super, "control"], "k", lazy.layout.shuffle_down(), desc="Move window down"),
    ...
    # tech
    Key([super, "shift"], "r", lazy.spawn('bash -c "killall polybar; polybar leftbar & polybar rightbar &"')), # restart polybar
    Key([super, "control"], "r", lazy.reload_config()), # restart qtile config
]
